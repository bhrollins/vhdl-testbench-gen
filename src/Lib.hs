...
import System.FilePath (takeFileName, dropExtension, FilePath)
...
generateTestbench opts = do
    ...
    if exists && not force then
        ...
        do
            putStrLn $ "Generating " ++ output ++ " from " ++ input
...
vhdlToTestbench progName outputPath (VhdlFile name generics ports) =
    ...
    where
        ...
        commentHeader = [ "-- scaffolding for `" ++ outputFile ++ "` generated by " ++ progName
        ...
        outputFile :: String
        outputFile = takeFileName outputPath

        tbName :: String
        tbName = dropExtension outputFile
